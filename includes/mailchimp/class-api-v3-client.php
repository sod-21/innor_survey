<?php
if ( !class_exists('MQZ_API_V3_Client') ) :
class MQZ_API_V3_Client {
	...
	*/
	private function request( $method, $resource, array $data = array() ) {
		...
		if ( empty( $this->api_key ) ) {
			throw new MQZ_API_Exception( 'Missing API key', 001 );
		}
		...
		$args   = array(
			...
			'timeout'   => 15,
			'sslverify' => apply_filters( 'MQZ_use_sslverify', true ),
		);
		...
		* @param array $args
		*/
		$args = apply_filters( 'MQZ_http_request_args', $args, $url );
	...
	*/
	private function get_headers() {
		...
		$headers['User-Agent']    = 'mc4wp/1.0; WordPress/' . $wp_version . '; ' . home_url();
	...
	* @return mixed
	*
	* @throws MQZ_API_Connection_Exception|MQZ_API_Resource_Not_Found_Exception|MQZ_API_Exception
	*/
	private function parse_response( $response ) {
		if ( $response instanceof WP_Error ) {
			throw new MQZ_API_Connection_Exception( $response->get_error_message(), (int) $response->get_error_code(), $this->last_request );
		}
		...
		if ( $code >= 400 ) {
			...
			if ( is_object( $data ) && isset( $data->type ) && $data->type === 'akamai_error_message' ) {
				throw new MQZ_API_Connection_Exception( $message, $code, $this->last_request, $this->last_response, $data );
			}

			if ( $code === 404 ) {
				throw new MQZ_API_Resource_Not_Found_Exception( $message, $code, $this->last_request, $this->last_response, $data );
			}

			// mailchimp returned an error..
			throw new MQZ_API_Exception( $message, $code, $this->last_request, $this->last_response, $data );
		}

		// throw exception if unable to decode response
		if ( $data === null ) {
			throw new MQZ_API_Exception( $message, $code, $this->last_request, $this->last_response );
		}
